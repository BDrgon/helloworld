...
class Gps:
    def __init__(self, bot):  # bot is the default robot object provided by MESA
        ...
        self.map = \
            {
                (0, 0): [  # The key is a 2-tuple of the coordinates of a square
                    [],  # The first list contains the squares that can be moved to from the key
                    [],  # The second list contains the squares that cannot be moved to from the key
                    [(0, 1), (0, -1), (1, 0), (-1, 0)]  # The third list contains the squares
                                                        # whose relation to the key is unknown
                ]
            }
        self.robot = bot  # Extend the default robot functions into nav.
        self.packets=bot.sense_packets() #sense_packets() returns dict{int: [float,float]} of packet locations
        self.clean_packets()
    ...
    def clean_packets(self): #convert each item in packets list to 2-tuples of ints from lists of floats
        for p in self.packets:
            self.packets[p] = (int(self.packets[p][0]),
                               int(self.packets[p][1]))
 ...
 # not sure if cleanup works: here is an alternative simpler method with the same goal
    ...
    def cleanup(self):  # this function guarantees each path or wall is two-way in the map
        ...
        for key in self.map:
            for point in self.map[key][2]:
                if point in self.map[key][0] + self.map[key][1]:
                    self.map[key][2].remove(point)
            if len(self.map[key][0] + self.map[key][1] + self.map[key][2]) > 4:
    ...
    def follow_path(self, path):  # This follows the path generated by djk
        for x in path:
            self.Vps.check_if_in_squares_sensed(x)
