class bag_o_functions:
    ...
    """
    def DJK(self, location, map, target): #full implementation of Dijkstra's algorithm over a dictionary object map
        ...
        infinity= float('inf') #this is a cheat number > all others in comparisons
        explored = {}
        ...
        frontier[location]= [[location],0]              #basic comparison with an actual connection regardless of size of found connection
        #setup complete
            ...
            if len(map[loc][0])> 0: #if there are nodes adjacent and connected to location
                for t in map[loc][0]: #iterate through these nodes
                    if t in unvisited: #select the node if it is unvisited
                        if  unvisited[t][1]>frontier[loc][1]+1: #if its tentative distance is smaller than its assigned distance
